...
def create_app(test_config=None):
    ...
    # Route and error handlers defined here
    @app.route('/movies', methods=['GET'])
    @requires_auth('view:movies')
    def retrieve_movies(payload):
        movies = Movie.query.all()
    ...
    def delete_movie(payload, movie_id):
        ...
        if movie is None:
            abort(404, f"No movie with id {movie_id} found")
    ...
    def delete_actor(payload, actor_id):
        ...
        if actor is None:
            abort(404, f"No actor with id {actor_id} found")
    ...
    def update_actor(payload, actor_id):
        ...
        try:
            ...
            abort(400, f"Bad request due to nonexistent movie id {movie_id}")
    ...
    @app.errorhandler(422)
    def unprocessable(error):
        return jsonify({"success": False, "error": 422, "message": get_error_message(error, "unprocessable")}), 422

    @app.errorhandler(404)
    def not_found(error):
        return jsonify({"success": False, "error": 404, "message": get_error_message(error, "resource not found")}), 404

    @app.errorhandler(400)
    def bad_request(error):
        return jsonify({"success": False, "error": 400, "message": get_error_message(error, "bad request")}), 400
